...
const Preparation = () => {
	...
	// const isBaked = metafields[10]?.value || "false";
	// const isFrozen = metafields[11]?.value || "true";
	...
	const matchedCity = cities.find((city: City) => city.name === selectedCity);
	const isBakedAvailable = matchedCity ? matchedCity?.baked : true;
	const isFrozenAvailable = matchedCity ? matchedCity?.frozen : state.selectionType === 'group-selection' ? false : true;
	...
	useEffect(() => {
		...
		if( delivery ) {
			...
			dispatch({
				type: "SET_STEP_DATA",
				payload: { stepIndex: state.currentStep, data: delivery },
			});
			...
			<div className="grid grid-cols-2 gap-8">
				...
				<div className="flex flex-col gap-3">
					<label htmlFor="Frozen" className={`
						...
						${!isFrozenAvailable ? "opacity-50 pointer-events-none" : ""}
					`}>
